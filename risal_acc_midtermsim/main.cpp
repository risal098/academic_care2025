...
int main() {
    ...
    vector<User> users=load_users(serialname);
        //if (users.empty()) { users.push_back(User()); }// fallback default
    ...
    while (true) {
        switch (prompt) {
            case MAIN_PROMPT:
                ...
                int choice;
                cin >> choice;
                prompt = static_cast<PrimaryPrompt>(choice - 1);//index dari 0 ;  static cast buat ubah primary jadi int
                break;
            case LOGIN:{
                ...
                switch (sub_login_prompt){
                    case USER_VERIFICATION:{
                        ...
                        bool isFound = false;
                        for(auto u: users){
                            cout<< "Checking user: " << u.getName() << ", " << u.getPhone() << endl;
                    ...
                    case LOGIN_MENU:{
                        ...
                        switch(login_menu_prompt){
                            ...
                            case SEEK_CONTACT:{
                                string query;
                                cout << "Please enter phone number / ids / name to search: ";
                                cin >> query;
                                bool isExist = user.checkUserExist(query);
                                if(isExist){
                                    ...
                                    cout << "Contact with character " << query << " is not found." << endl;
                                }
                                cout << endl;
                                login_menu_prompt = LOGIN_MENU_PROMPT;
                                break;
            ...
            case REGISTER:{
                ...
                if(users.size()>0){
                    ...
                    cout<< "Setting this user id to: " << User::getId() << endl;
                }
                ...
                if(!isFound){
                    cout << "entered info acceptable" << endl;
                    users.push_back(user);
            ...
            case EXIT:
                ...
                if(users.size()>0){
                save_users(users, serialname);
                cout << "Exiting the program. Goodbye!" << endl;
                } else {
                    cout << "No user data to save. Exiting the program. Goodbye!" << endl;
                }
                return 1;
        }
    }
